...
import { MdCheckCircle, MdAssignmentInd, MdCloudDone } from 'react-icons/md'
import { RiSteering2Line } from 'react-icons/ri'
import { getBadgeColor, getDotColor } from '../pages/Home/Components/estadoColors.js'

export default function Toast({ tipo = 'estado', estado, asignadoA, cantidad = 0, onClose }) {
  useEffect(() => {
  ...
  const esSync = tipo === 'sync'

  const text = esSync
    ? `Conexión restablecida: ${cantidad} entrega(s) sincronizada(s)`
    : esConductor
  ...
  const colorClass = esSync
    ? 'bg-green-600 text-white'
    : esAsignacion || esConductor
      ? 'bg-brand-navy text-white'
      : estado
        ? getBadgeColor(estado)
        : 'bg-brand-cyan text-brand-ink'
  const dotClass = esSync || esAsignacion || esConductor ? 'bg-brand-cyan' : estado ? getDotColor(estado) : 'bg-brand-ink'
  ...
  return (
    <div className="fixed inset-x-0 top-4 z-[1100] flex justify-center px-4 animate-slideDown" role="status" aria-live="polite">
      ...
      >
        ...
        {esSync ? (
          <MdCloudDone className="text-lg text-brand-cyan" />
        ) : esConductor ? (
          <RiSteering2Line className="text-lg text-brand-cyan" />
        ) : esAsignacion ? (
