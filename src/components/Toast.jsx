...
export default function Toast({ tipo = 'estado', estado, asignadoA, onClose }) {
  ...
  const text = esAsignacion
    ? `Solicitud asignada a «${asignadoA || 'Sin asignar'}»`
    : `Estado actualizado a «${estado || '—'}»`

  const colorClass = esAsignacion ? 'bg-brand-navy text-white' : estado ? getBadgeColor(estado) : 'bg-brand-cyan text-brand-ink'
  const dotClass = esAsignacion ? 'bg-brand-cyan' : estado ? getDotColor(estado) : 'bg-brand-ink'
  ...
  return (
    <div className="fixed inset-x-0 top-4 z-[1100] flex justify-center px-4 animate-slideDown" role="status" aria-live="polite">
      ...
      >
        ...
        {esAsignacion ? <MdAssignmentInd className="text-lg text-brand-cyan" /> : <MdCheckCircle className="text-lg" />}
        {text}
